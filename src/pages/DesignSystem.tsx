...
function Category({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="flex flex-col gap-6">
      <h2 className="text-xl font-bold text-text border-b border-slate-200 dark:border-slate-700 pb-2">{title}</h2>
      {children}
    </section>
  )
}
      ...
      <Group>
        ...
        <div className="flex flex-col gap-2 max-w-xs">
          {(['teal', 'indigo', 'amber', 'green', 'rose', 'pink', 'red'] as const).map((color) => (
            <HomeButton key={color} to="#" color={color}>
              {color}
            </HomeButton>
          ))}
        </div>
  ...
  return (
    <Stack>
      <Group>
        ...
        <NumberInput placeholder="0" value={value} onChange={(e) => setValue(e.target.value)} focusColor="teal" />
      </Group>
      <Group>
        <Title>NumberInput — indigo focus (multiplication)</Title>
        <NumberInput placeholder="0" defaultValue="" focusColor="indigo" readOnly />
      </Group>
      <Group>
        <Title>NumberInput — error state</Title>
        <NumberInput defaultValue="abc" readOnly error />
    ...
    <div className="flex gap-2">
      {fixtures.map(({ label, completedCount }) => (
        <div key={label} className="flex-1 flex flex-col items-center gap-2">
          <LongDivisionDisplay dividend={657} divisor={3} steps={steps} completedCount={completedCount} />
          <Label>{label}</Label>
        </div>
      ))}
    </div>
    ...
    <div className="flex flex-col gap-6">
      {amFixtures.map(({ label, sections, remaining }) => (
        <div key={label} className="flex flex-col gap-2">
          <Label>{label}</Label>
          <AreaModelRect dividend={630} divisor={7} sections={sections} remaining={remaining} />
        </div>
      ))}
    </div>
  )
