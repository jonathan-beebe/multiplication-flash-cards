...
export function DrillCompleteContent({
  correctCount = 0,
  wrongCount = 0,
  onRestart,
}: {
  ...
  return (
    <div className="flex w-full max-w-md flex-col items-center gap-8 text-center">
      <h1 className="text-4xl font-bold text-text">Drill Complete!</h1>
      <div className="flex flex-col gap-4 rounded-2xl border-2 border-slate-200 bg-white p-8 text-left shadow-lg dark:border-slate-700 dark:bg-slate-800">
        <div className="text-2xl text-slate-700 dark:text-slate-300">
          <span className="font-bold text-text">{correctCount + wrongCount}</span> attempted
        </div>
        <div className="text-2xl text-slate-700 dark:text-slate-300">
          <span className="font-bold text-green-600 dark:text-green-400">{correctCount}</span> correct
        </div>
        <div className="text-2xl text-slate-700 dark:text-slate-300">
          <span className="font-bold text-red-600 dark:text-red-400">{wrongCount}</span> wrong
        </div>
      </div>
...
function DrillComplete({ correctCount: propCorrect, wrongCount: propWrong }: DrillCompleteProps) {
  ...
  const onRestart = location.state ? () => navigate('..', { relative: 'path', replace: true }) : undefined

  return (
    <main className="flex min-h-screen items-center justify-center px-4 bg-background">
      <DrillCompleteContent correctCount={correctCount} wrongCount={wrongCount} onRestart={onRestart} />
    </main>
  )
}
