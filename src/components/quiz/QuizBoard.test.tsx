...
describe('QuizBoard', () => {
  ...
  it('renders a question and 3 choice buttons', () => {
    render(
      <QuizBoard generator={mockGenerator} getNextQuestion={mockGetNextQuestion} renderQuestion={renderQuestion} />,
    )
  ...
  it('calls onWrong when a wrong answer is clicked', async () => {
    ...
    const correctAnswer = getCorrectAnswer()
    const wrongButton = screen.getAllByRole('button').find((btn) => btn.textContent !== String(correctAnswer))!
